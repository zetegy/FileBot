...
public final class FSEventStream {
    ...
    private let handler: (FSEventsEvent) -> ()
     ...
     */
    public init(pathsToWatch: [String], sinceWhen: FSEventsEventID, latency: TimeInterval, flags: FSEventsCreateFlags, handler: @escaping (FSEventsEvent) -> ()) throws {
        ...
        let callback: CoreServices.FSEventStreamCallback = { (
            ...
            guard let clientCallBackInfo1 = clientCallBackInfo else {
                FSEventsIllogicalErrorLog(code: .missingContextRawPointerValue).cast()
                return
            }
            ...
            guard let eventPaths2 = eventPaths1 as NSArray as? [NSString] as [String]? else {
                FSEventsIllogicalErrorLog(code: .unexpectedPathValueType, message: "Cannot convert `\(eventPaths1)` into [String].").cast()
                return
            }
            guard numEvents == eventPaths2.count else {
                FSEventsIllogicalErrorLog(code: .unmatchedEventParameterCounts, message: "Event count is `\(numEvents)`, but path count is `\(eventPaths2.count)`").cast()
                return
            }
            let unmanagedPtr: Unmanaged<FSEventStream> = Unmanaged.fromOpaque(clientCallBackInfo1)
            let self1 = unmanagedPtr.takeUnretainedValue()
            for i in 0..<numEvents {
                let eventPath = eventPaths2[i]
                let eventFlag = eventFlags[i]
                let eventFlag1 = FSEventsEventFlags(rawValue: eventFlag)
                let eventId = eventIds[i]
                let eventId1 = FSEventsEventID(rawValue: eventId)
                let event = FSEventsEvent(path: eventPath,
                                                                flag: eventFlag1,
                                                                ID: eventId1)
                self1.handler(event)
            }
        ...
        guard let newRawref = FSEventStreamCreate(nil, callback, context1, pathsToWatch1, sinceWhen1, latency1, flags1) else {
            throw FSEventsError(code: .cannotCreateStream)
        }
        rawref = newRawref
    }
    deinit {
...
extension FSEventStream {
    ...
    public func getLatestEventID() -> FSEventsEventID {
...
//    }
    ...
    public func copyPathsBeingWatched() -> [String] {
        ...
        guard let paths = ret as NSArray as? [NSString] as [String]? else {
            FSEventsIllogicalErrorLog(code: .unexpectedPathValueType, message: "Cannot convert retrieved object `\(ret)` into `[String]`.").cast()
            // Unrecoverable.
            fatalError()
        }
        return paths
...
extension FSEventStream {
    ...
    public func start() throws {
        ...
        case false:
            throw FSEventsError.init(code: .cannotStartStream)
        case true:
            return
        }
    }
     ...
     */
    @available(OSX 10.5, *)
    public func flushAsync() -> FSEventsEventID {
...
extension FSEventStream: CustomStringConvertible, CustomDebugStringConvertible {
